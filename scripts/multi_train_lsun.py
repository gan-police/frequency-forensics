...
experiment_lst = []
for model in ['cnn']:
    for wavelet_str in ["sym2", "sym3", "sym4", "sym5", "db3", "db4", "db5"]:
        for seed in [0, 1, 2, 3, 4]:
            experiment_lst.append(
                (["python", "-m", "freqdect.train_classifier", "--features", "packets",
                 ...
                 "--seed", str(seed),
                 "--data-prefix",
                 "/nvme/mwolter/lsun/lsun_bedroom_200k_png_log_packets_" + wavelet_str + "_boundary",
...
for exp_no, experiment in enumerate(experiment_lst):
    ...
    file_name = f"{time_str}_lsun_{experiment[1][0]}_{experiment[1][1]}_{experiment[1][2]}.txt"
